...
def fix_values(data: dict[dict]) -> None:
    ...
    for key, value in data.items():
        if not value:
            ...
            if math:
                ...
                new_items[key[:delimiter_pos].strip()] = (
                    key[delimiter_pos + 1:].strip())
            else:
                new_items[key.strip()] = ''
    ...
    if len(keys_to_remove) != len(new_items.keys()):
        raise KeyError(f'Ошибка при создании словаря. Дублируются ключи: '
                       f'{str(keys_to_remove)}')
...
def extract_to_dict(
        ...
        start_pattern: str = None, end_pattern: str = None
) -> tuple[dict, int]:
    ...
    stack = [(result, -1)]
    if start_pattern:
        for line, start_line in zip(lines[start_line:],
                                range(start_line, len(lines) + 1)):
            if re.match(start_pattern, line):
                break

    for line, number in zip(lines[start_line:],
                            range(start_line, len(lines)+1)):
...
def extract_conf(file: BytesIO) -> dict[str, dict]:
    with (tarfile.open(fileobj=file) as tar):
        ...
        with tarfile.open(fileobj=config_tar) as conf_tar:
            ...
            conf_by_line = conf_tar.extractfile('config.txt').read(
            ).decode("utf-8").split('\n')
    summary, key = extract_to_dict(
        conf_by_line,
        start_pattern='^.{0,}SUMMARY--', end_pattern=r'^.{0,}License--'
    )
    print(key)
    result = {
...
def make_dict_tree(data):
    dict_data = {}
    for line in data:
        for item in line.split('->'):
            name_list = item.split('.')
            current = dict_data
            for key, depth in zip(name_list, range(len(name_list), 0, -1)):
