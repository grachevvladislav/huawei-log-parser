...
if 'curr_state' not in st.session_state:
    nodes = [
        ...
        StreamlitFlowNode("2", (1, 0), {'content': 'Node 2'}, 'default',
                          'right', 'left'),
    ...
    edges = [StreamlitFlowEdge("1-2", "1", "2", animated=True, marker_start={},
...
with col2:
    if st.button("Delete Random Node"):
        if len(st.session_state.curr_state.nodes) > 0:
            ...
            st.session_state.curr_state.nodes = [node for node in
                                                 st.session_state.curr_state.nodes
                                                 if
                                                 node.id != node_to_delete.id]
            st.session_state.curr_state.edges = [edge for edge in
    ...
    if st.button("Add Edge"):
        if len(st.session_state.curr_state.nodes) > 1:
            ...
            target = random.choice(
                [node for node in st.session_state.curr_state.nodes if
                 node.id != source.id])
            new_edge = StreamlitFlowEdge(f"{source.id}-{target.id}", source.id,
                                         target.id, animated=True)
